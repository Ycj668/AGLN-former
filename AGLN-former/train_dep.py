...
from model import Generator, Discriminator,  mytEEGTransformer
...
def LOCO(sub, clip_num, learning_rate, batch_size, epoch):
    ...
    with open(config.save_index_1 + '/result_LOCO_1/result_' + sub + '_' + 'clip' + str(clip_num) + '.txt', 'w') as f:
        ...
        net = mytEEGTransformer(C,F, 4, 1)    
        net.to(device)

        train_loss, train_acc, test_loss, test_acc, f_score, RECALL_score, PRE_score, \
            = traink(net, train_loader, test_loader, learning_rate, batch_size, epoch, sub, clip_num)
        ...
        torch.save(net.state_dict(),
                   config.save_index_1 + '/result_LOCO_1/trained_classfication_' + sub + '_' + 'clip' + str(clip_num) + '.pt')
...
for sub in config.subjectList:
    for clip_num in config.clip_num_list:
        with open(config.save_index_1 + '/result_LOCO_1/result_' + sub + '_' + 'clip' + str(clip_num) + '.txt', 'w') as f:
            ...
            train_acc, test_acc, test_f1_score\
