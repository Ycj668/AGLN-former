    ...
    for clip_num in config.clip_num_list:  
        ...
        for epoch in range(config.gan_epoch):  
            ...
            for i, (data, sample) in enumerate(zip(train_loader, sample_loader)):
                ...
                real_label = Variable(torch.ones(BATCHSIZE, dtype=torch.int64)).to(device)    
                fake_label = Variable(torch.zeros(BATCHSIZE, dtype=torch.int64)).to(device)  
                ...
                sample = sample.view(BATCHSIZE, -1)   
